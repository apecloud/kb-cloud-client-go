...
// InstancesToOffline Take instances offline.
func (a *OpsrequestApi) InstancesToOffline(ctx _context.Context, orgName string, clusterName string, body OpsHScale) (OpsRequestName, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue OpsRequestName
	...
	apiInfo := common.APIInfo{
		Tag:         "opsrequest",
		OperationID: "instancesToOffline",
		Path:        "/api/v1/organizations/{orgName}/clusters/{clusterName}/instancesToOffline",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".OpsrequestApi.InstancesToOffline")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/instancesToOffline"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
