...
func (r *ListNodesOptionalParameters) WithZones(zones []string) *ListNodesOptionalParameters {
	r.Zones = &zones
	return r
}

// ListNodes List environment nodes.
// Retrieve a list of nodes for the specified environment
func (a *EnvironmentApi) ListNodes(ctx _context.Context, orgName string, environmentName string, o ...ListNodesOptionalParameters) (NodeList, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
	...
	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/environments/{environmentName}/nodeList"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"environmentName"+"}", _neturl.PathEscape(common.ParameterToString(environmentName, "")), -1)
