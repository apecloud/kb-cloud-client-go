...
// ListKoordinatorReservations List Koordinator Reservations.
// Get Koordinator Reservation summary for the cluster creation page.
func (a *EnvironmentApi) ListKoordinatorReservations(ctx _context.Context, orgName string, environmentName string) (KoordinatorReservationSummary, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/api/v1/organizations/{orgName}/environments/{environmentName}/koordinatorReservations/summary",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".EnvironmentApi.ListKoordinatorReservations")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/environments/{environmentName}/koordinatorReservations/summary"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"environmentName"+"}", _neturl.PathEscape(common.ParameterToString(environmentName, "")), -1)
