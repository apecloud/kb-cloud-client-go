...
// GetEnvironmentPersistentVolumeClaim Get a live PersistentVolumeClaim.
// Returns the Kubernetes v1 PersistentVolumeClaim object, excluding metadata.managedFields. Requires environment read permission in the Admin Portal.
func (a *EnvironmentApi) GetEnvironmentPersistentVolumeClaim(ctx _context.Context, environmentName string, namespace string, pvcName string) (map[string]interface{}, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		localVarReturnValue map[string]interface{}
	...
	apiInfo := common.APIInfo{
		Tag:         "environment",
		OperationID: "getEnvironmentPersistentVolumeClaim",
		Path:        "/admin/v1/environments/{environmentName}/namespaces/{namespace}/pvcs/{pvcName}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".EnvironmentApi.GetEnvironmentPersistentVolumeClaim")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/environments/{environmentName}/namespaces/{namespace}/pvcs/{pvcName}"
	localVarPath = strings.Replace(localVarPath, "{"+"environmentName"+"}", _neturl.PathEscape(common.ParameterToString(environmentName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"namespace"+"}", _neturl.PathEscape(common.ParameterToString(namespace, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"pvcName"+"}", _neturl.PathEscape(common.ParameterToString(pvcName, "")), -1)
