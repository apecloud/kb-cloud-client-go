...
// GetEnvironmentResourceOptionalParameters holds optional parameters for GetEnvironmentResource.
type GetEnvironmentResourceOptionalParameters struct {
	Namespace *string
}

// NewGetEnvironmentResourceOptionalParameters creates an empty struct for parameters.
func NewGetEnvironmentResourceOptionalParameters() *GetEnvironmentResourceOptionalParameters {
	this := GetEnvironmentResourceOptionalParameters{}
	return &this
}

// WithNamespace sets the corresponding parameter name and returns the struct.
func (r *GetEnvironmentResourceOptionalParameters) WithNamespace(namespace string) *GetEnvironmentResourceOptionalParameters {
...
func (a *EnvironmentApi) GetEnvironmentResource(ctx _context.Context, environmentName string, resourceName string, apiVersion string, kind string, o ...GetEnvironmentResourceOptionalParameters) (map[string]interface{}, *_nethttp.Response, error) {
	var (
		...
		localVarPostBody    interface{}
		localVarReturnValue map[string]interface{}
		optionalParams      GetEnvironmentResourceOptionalParameters
	)

	if len(o) > 1 {
		return localVarReturnValue, nil, common.ReportError("only one argument of type GetEnvironmentResourceOptionalParameters is allowed")
	}
	if len(o) == 1 {
		optionalParams = o[0]
	}
	...
	apiInfo := common.APIInfo{
		Tag:         "environment",
		OperationID: "getEnvironmentResource",
		Path:        "/admin/v1/environments/{environmentName}/resources/{resourceName}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".EnvironmentApi.GetEnvironmentResource")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/environments/{environmentName}/resources/{resourceName}"
	localVarPath = strings.Replace(localVarPath, "{"+"environmentName"+"}", _neturl.PathEscape(common.ParameterToString(environmentName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"resourceName"+"}", _neturl.PathEscape(common.ParameterToString(resourceName, "")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	localVarQueryParams.Add("apiVersion", common.ParameterToString(apiVersion, ""))
	localVarQueryParams.Add("kind", common.ParameterToString(kind, ""))
