...
// ResetDiagnosticsPostgresqlSQLAnalysis Reset PostgreSQL SQL analysis statistics.
// Reset pg_stat_statements statistics for the selected PostgreSQL cluster. This is a cluster write operation and does not change PostgreSQL configuration.
func (a *DiagnosticsApi) ResetDiagnosticsPostgresqlSQLAnalysis(ctx _context.Context, orgName string, clusterName string) (*_nethttp.Response, error) {
	var (
	...
	apiInfo := common.APIInfo{
		Tag:         "diagnostics",
		OperationID: "resetDiagnosticsPostgresqlSQLAnalysis",
		Path:        "/admin/v1/organizations/{orgName}/clusters/{clusterName}/diagnostics/postgresql/sqlAnalysis/reset",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".DiagnosticsApi.ResetDiagnosticsPostgresqlSQLAnalysis")
	if err != nil {
		return nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/organizations/{orgName}/clusters/{clusterName}/diagnostics/postgresql/sqlAnalysis/reset"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
