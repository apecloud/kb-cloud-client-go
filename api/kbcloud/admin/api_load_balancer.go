...
// GetLoadBalancerConfig Get the effective load balancer configuration of an environment.
func (a *LoadBalancerApi) GetLoadBalancerConfig(ctx _context.Context, environmentName string) (LoadBalancerConfig, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		localVarReturnValue LoadBalancerConfig
	)
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/admin/v1/environments/{environmentName}/loadbalancer/config",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".LoadBalancerApi.GetLoadBalancerConfig")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/environments/{environmentName}/loadbalancer/config"
	localVarPath = strings.Replace(localVarPath, "{"+"environmentName"+"}", _neturl.PathEscape(common.ParameterToString(environmentName, "")), -1)
...
// ResetLoadBalancerConfig Reset the environment load balancer configuration to the current provider defaults.
func (a *LoadBalancerApi) ResetLoadBalancerConfig(ctx _context.Context, environmentName string) (LoadBalancerConfig, *_nethttp.Response, error) {
	var (
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".LoadBalancerApi.ResetLoadBalancerConfig")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}
...
// UpdateLoadBalancerConfig Save and immediately activate the load balancer configuration of an environment.
func (a *LoadBalancerApi) UpdateLoadBalancerConfig(ctx _context.Context, environmentName string, body LoadBalancerConfigUpdate) (LoadBalancerConfig, *_nethttp.Response, error) {
	var (
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".LoadBalancerApi.UpdateLoadBalancerConfig")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}
