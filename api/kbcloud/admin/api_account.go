...
func (a *AccountApi) DeleteMssqlAccount(ctx _context.Context, orgName string, clusterName string, accountName string) (ClusterTask, *_nethttp.Response, error) {
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/admin/v1/data/mssql/organizations/{orgName}/clusters/{clusterName}/accounts/{accountName}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)
	...
	localVarPath := localBasePath + "/admin/v1/data/mssql/organizations/{orgName}/clusters/{clusterName}/accounts/{accountName}"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"accountName"+"}", _neturl.PathEscape(common.ParameterToString(accountName, "")), -1)
