...
func (a *RedisApi) DisableRedisInsight(ctx _context.Context, orgName string, clusterName string) (*_nethttp.Response, error) {
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/admin/v1/data/redis/organizations/{orgName}/clusters/{clusterName}/redisInsight",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)
	...
	localVarPath := localBasePath + "/admin/v1/data/redis/organizations/{orgName}/clusters/{clusterName}/redisInsight"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
