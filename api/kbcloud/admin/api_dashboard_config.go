...
// DeleteDashboardConfig Delete dashboard configuration by key.
// Requires platform configuration write access. Deleting a missing key succeeds.
func (a *DashboardConfigApi) DeleteDashboardConfig(ctx _context.Context, key string) (*_nethttp.Response, error) {
	var (
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/admin/v1/dashboardConfigs/{key}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".DashboardConfigApi.DeleteDashboardConfig")
	if err != nil {
		return nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/dashboardConfigs/{key}"
	...
	if common.Strlen(key) < 1 {
		return nil, common.ReportError("key must have at least 1 elements")
	}
	if common.Strlen(key) > 512 {
		return nil, common.ReportError("key must have less than 512 elements")
	}
...
// PutDashboardConfig Create or replace dashboard configuration by key.
...
func (a *DashboardConfigApi) PutDashboardConfig(ctx _context.Context, key string, body DashboardConfigUpdate) (DashboardConfig, *_nethttp.Response, error) {
	var (
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".DashboardConfigApi.PutDashboardConfig")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}
