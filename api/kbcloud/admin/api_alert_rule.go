...
// NewCreateAlertRuleOptionalParameters creates an empty struct for parameters.
func NewCreateAlertRuleOptionalParameters() *CreateAlertRuleOptionalParameters {
	this := CreateAlertRuleOptionalParameters{}
	return &this
}
...
func (a *AlertRuleApi) CreateAlertRule(ctx _context.Context, orgName string, body AlertRule, o ...CreateAlertRuleOptionalParameters) (AlertRule, *_nethttp.Response, error) {
	var (
		...
		optionalParams      CreateAlertRuleOptionalParameters
	)

	if len(o) > 1 {
		return localVarReturnValue, nil, common.ReportError("only one argument of type CreateAlertRuleOptionalParameters is allowed")
	}
	if len(o) == 1 {
		optionalParams = o[0]
	}
...
func (a *AlertRuleApi) DeleteAlertRule(ctx _context.Context, orgName string, alertName string) (interface{}, *_nethttp.Response, error) {
	...
	localVarPath := localBasePath + "/admin/v1/organizations/{orgName}/alerts/rules/{alertName}"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"alertName"+"}", _neturl.PathEscape(common.ParameterToString(alertName, "")), -1)
...
// GetRuleConfig Get alert rule configuration YAML.
// Retrieves the complete alert rule configuration in YAML format for all organizations
func (a *AlertRuleApi) GetRuleConfig(ctx _context.Context) (string, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/admin/v1/alerts/rules/config",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)
	...
	localVarPath := localBasePath + "/admin/v1/alerts/rules/config"
