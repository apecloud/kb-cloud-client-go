...
// MetadbHorizontalScale metadb horizontal scale.
func (a *MetadbApi) MetadbHorizontalScale(ctx _context.Context, body Hscale) (Hscale, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue Hscale
	)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".MetadbApi.MetadbHorizontalScale")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/admin/v1/metadb/hscale"
