...
// MetadbCheckBackupConfig check backup config.
func (a *MetadbApi) MetadbCheckBackupConfig(ctx _context.Context, body BackupConfig) (BackupConfig, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue BackupConfig
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".MetadbApi.MetadbCheckBackupConfig")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}
