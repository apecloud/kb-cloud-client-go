...
// GetClusterSecrets Get cluster secrets.
// get secrets in cluster
func (a *ClusterApi) GetClusterSecrets(ctx _context.Context, orgName string, clusterName string) (SecretList, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".ClusterApi.GetClusterSecrets")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/secrets"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
