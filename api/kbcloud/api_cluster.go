...
func (a *ClusterApi) GetClusterManifest(ctx _context.Context, orgName string, clusterName string, manifestType ManifestType, o ...GetClusterManifestOptionalParameters) (KubernetesManifestList, *_nethttp.Response, error) {
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/api/v1/organizations/{orgName}/clusters/{clusterName}/manifests/{manifestType}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)
	...
	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/manifests/{manifestType}"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"manifestType"+"}", _neturl.PathEscape(common.ParameterToString(manifestType, "")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	if optionalParams.OpsType != nil {
		localVarQueryParams.Add("opsType", common.ParameterToString(*optionalParams.OpsType, ""))
	}
