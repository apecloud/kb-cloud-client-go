...
// BindDNS Bind external DNS to a cluster component.
func (a *ClusterApi) BindDNS(ctx _context.Context, orgName string, clusterName string, body DnsBindRequest) (ClusterDNSBinding, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue ClusterDNSBinding
	)
	...
	apiInfo := common.APIInfo{
		Tag:         "cluster",
		OperationID: "bindDNS",
		Path:        "/api/v1/organizations/{orgName}/clusters/{clusterName}/dns/bind",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".ClusterApi.BindDNS")
	if err != nil {
		return localVarReturnValue, nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/dns/bind"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
...
// UnbindDNS Unbind external DNS from a cluster component.
func (a *ClusterApi) UnbindDNS(ctx _context.Context, orgName string, clusterName string, component string) (*_nethttp.Response, error) {
	var (
		localVarHTTPMethod = _nethttp.MethodDelete
		localVarPostBody   interface{}
	)
	...
	apiInfo := common.APIInfo{
		Tag:         "cluster",
		OperationID: "unbindDNS",
		Path:        "/api/v1/organizations/{orgName}/clusters/{clusterName}/dns/unbind",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".ClusterApi.UnbindDNS")
	if err != nil {
		return nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/dns/unbind"
	localVarPath = strings.Replace(localVarPath, "{"+"orgName"+"}", _neturl.PathEscape(common.ParameterToString(orgName, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"clusterName"+"}", _neturl.PathEscape(common.ParameterToString(clusterName, "")), -1)
