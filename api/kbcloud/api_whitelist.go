...
func (a *WhitelistApi) DeleteIPWhiteList(ctx _context.Context, orgName string, clusterName string, ipWhitelistId string) (*_nethttp.Response, error) {
	...
	apiInfo := common.APIInfo{
		...
		Path:        "/api/v1/organizations/{orgName}/clusters/{clusterName}/ipWhitelist/{ipWhitelistId}",
		Version:     "",
	}
	ctx = context.WithValue(ctx, common.APIInfoCtxKey, apiInfo)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, ".WhitelistApi.DeleteIPWhiteList")
	if err != nil {
		return nil, common.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/organizations/{orgName}/clusters/{clusterName}/ipWhitelist/{ipWhitelistId}"
