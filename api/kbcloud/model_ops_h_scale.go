...
type OpsHScale struct {
	...
	// Target replica count for one or more instance templates. Required for replica hscale when the engine option declares instanceTemplate with hscale. Mutually exclusive with shards. Top-level replicas does not override these counts; KB ops fill component replicaChanges and per-template instances together. Names must be unique and declared; unspecified templates are left unchanged.
