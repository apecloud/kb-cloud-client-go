...
def get_enum_varname(schema, index):
...
def simple_type(schema, render_nullable=False, render_new=False):
    ...
    if type_name == "integer":
        return {
            "int16": "int16" if not nullable else f"{nullable_prefix}Int16",
...
def reference_to_value(schema, value, print_nullable=True, **kwargs):
    ...
    if type_name in PRIMITIVE_TYPES:
        ...
        prefix = f"common{kwargs.get('version', '')}."
    ...
    **kwargs,
    ...
    if "enum" in schema and name:
        if data is None and nullable:
            ...
            enum_varname = get_enum_varname(schema, index)
            parameters = f"{name_prefix}{name.upper()}_{enum_varname}"
...
def format_data_with_schema_list(
    ...
    **kwargs,
    ...
    if nullable and is_primitive(list_schema):
        name_prefix = "common.NewNullableList"
